...
#define CONFIG_I2CDEV_TIMEOUT 5000

static const char *TAG = "I2C DEV";
...
esp_err_t i2c_dev_write_8(const i2c_dev_t *dev, int _chip_addr, uint8_t data_addr, uint8_t data)
{
    SEMAPHORE_TAKE(dev->port);
    esp_err_t res = i2c_setup_port(dev);
    if (res == ESP_OK)
    {
        i2c_cmd_handle_t cmd = i2c_cmd_link_create();
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, _chip_addr<< 1 | I2C_MASTER_WRITE, true);
        i2c_master_write_byte(cmd, data_addr, true);
        i2c_master_write_byte(cmd, data, true);
        i2c_master_stop(cmd);
        res = i2c_master_cmd_begin(dev->port, cmd, pdMS_TO_TICKS(CONFIG_I2CDEV_TIMEOUT));
        if (res != ESP_OK)
            ESP_LOGE(TAG, "Could not write to device [0x%02x at %d]: %d (%s)", _chip_addr, dev->port, res, esp_err_to_name(res));
        i2c_cmd_link_delete(cmd);
        usleep(1000*2);
    }
    SEMAPHORE_GIVE(dev->port);
...
esp_err_t i2c_dev_read_8(const i2c_dev_t *dev, int _chip_addr, uint8_t data_addr, uint8_t *data)
{
    SEMAPHORE_TAKE(dev->port);
    esp_err_t res = i2c_setup_port(dev);
    if (res == ESP_OK)
    {
        ...
        i2c_master_write_byte(cmd, data_addr, true);
        i2c_master_start(cmd);
        i2c_master_write_byte(cmd, _chip_addr << 1 | I2C_MASTER_READ, true);
...
esp_err_t i2c_dev_read_16(const i2c_dev_t *dev, uint16_t data_addr, uint8_t * data)
{
    //if (!dev || !in_data || !in_size) return ESP_ERR_INVALID_ARG;
    ...
    if (res == ESP_OK)
    {
        ...
        i2c_master_write_byte(cmd, low_addr, true);
        i2c_master_start(cmd);
...
esp_err_t i2c_dev_write_16(const i2c_dev_t *dev, uint16_t data_addr, uint8_t data)
{
    //if (!dev || !out_data || !out_size) return ESP_ERR_INVALID_ARG;
    ...
    if (res == ESP_OK)
    {
        ...
        // if (out_reg && out_reg_size)
        //     i2c_master_write(cmd, (void *)out_reg, out_reg_size, true);
        // i2c_master_write(cmd, (void *)out_data, out_size, true);s
